...
function triggerNextEvent() {
  ...
  const shuffled = ev.choices.map((c, i) => ({ ...c, origIdx: i })).sort(() => Math.random() - 0.5);
  shuffled.forEach((c, i) => {
    const btn = document.createElement('button');
    btn.className = 'choice-btn';
    btn.innerHTML = `<span class="choice-num">${i + 1}</span><span>${c.text}</span>`;
    btn.onclick = () => makeChoice(c.origIdx, c, false);
...
function makeChoice(idx, choice) {
  ...
  state.social = Math.max(0, Math.min(100, state.social + choice.social));
  state.distance = Math.max(0, state.distance - choice.time);
  state.totalScore += choice.score;
  ...
  if (choice.social !== 0) ef.push(`<span class="effect-chip ${choice.social < 0 ? 'effect-bad' : 'effect-good'}">${choice.social > 0 ? '+' : ''}${choice.social} IMPACTO</span>`);
  if (choice.time > 0) ef.push(`<span class="effect-chip effect-warn">-${choice.time}m NO PERCURSO</span>`);
  if (choice.score !== 0) ef.push(`<span class="effect-chip ${choice.score < 0 ? 'effect-bad' : 'effect-good'}">${choice.score > 0 ? '+' : ''}${choice.score} PONTOS</span>`);
...
function continueGame() {
  ...
  state.nextEventAt = state.distance + 50 + Math.random() * 40;
}
...
function saveAndRanking() {
  if (!state.scoreSaved) {
    ...
    scores.push({ name: playerName, time: totalGameTime, score: state.totalScore, social: state.social });
